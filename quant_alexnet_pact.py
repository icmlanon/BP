...
class AlexNet_Quant_PACT(nn.Module):

    def __init__(self, num_classes=10):
        super(AlexNet_Quant_PACT, self).__init__()
        self.features = nn.Sequential(
            #nn.Conv2d(3, 64, kernel_size=11, stride=4, padding=5),
            Quant_Conv2d(3, 64, kernel_size=11, stride=4, padding=5, quantize=quantize, interpolate=interpolate, bits_w=bits_w, bits_a=bits_a),
            PACT(),
        ...
        self.classifier = Quant_IP(256, num_classes, quantize=quantize, interpolate=interpolate, bits_w=bits_w, bits_a=bits_a)
...
def quant_alexnet_PACT(pretrained=False, **kwargs):
    """AlexNet model architecture from the
    `"One weird trick..." <https://arxiv.org/abs/1404.5997>`_ paper.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = AlexNet_Quant_PACT(**kwargs)
    if pretrained:
